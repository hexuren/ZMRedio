...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(PlayNext:) name:@"PlayNext" object:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [CommonMethod writeToUserDefaults:@(indexPath.row) withKey:@"currentPlayIndex"];
    [CommonMethod writeToUserDefaults:self.tracksVM withKey:@"currentTracksVM"];
...
- (void)PlayNext:(NSNotification *)notice{
    
    NSInteger currentIndex = [[CommonMethod readFromUserDefaults:@"currentPlayIndex"] integerValue];
    self.tracksVM = [CommonMethod readFromUserDefaults:@"currentTracksVM"];
    if (currentIndex < self.tracksVM.rowNumber) {
        currentIndex ++;
        [CommonMethod writeToUserDefaults:@(currentIndex) withKey:@"currentPlayIndex"];
        // 通知按钮旋转,播放及按钮改变
        NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
        userInfo[@"coverURL"] = [self.tracksVM coverURLForRow:currentIndex];
        userInfo[@"musicURL"] = [self.tracksVM playURLForRow:currentIndex];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"BeginPlay" object:nil userInfo:[userInfo copy]];
        
//        NSURL *musicURL = [self.tracksVM playURLForRow:currentIndex];;
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        frame.origin.y += 20;
        frame.size.height = self.view.frame.size.height - 20 - 70;
        // 设置普通模式
