...
- (UIButton *)topLeftBtn {
    if (!_topLeftBtn) {
        ...
        [_topLeftBtn bk_addEventHandler:^(id sender) {
            if ([weakSelf.delegete respondsToSelector:@selector(topLeftButtonDidClick)]) {
                [weakSelf.delegete topLeftButtonDidClick];
            }
...
- (UIButton *)topRightBtn {
    if (!_topRightBtn) {
        ...
        [_topRightBtn bk_addEventHandler:^(id sender) {
            if ([weakSelf.delegete respondsToSelector:@selector(topRightButtonDidClick)]) {
                // 代理需要做的方法
                [weakSelf.delegete topRightButtonDidClick];
            }
        } forControlEvents:UIControlEventTouchUpInside];
    }
    return _topRightBtn;
