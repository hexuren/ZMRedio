...
//    [APPRequest getTracksForAlbumId:_albumId mainTitle:_title idAsc:_asc successBlock:^(NSURLSessionDataTask *dataTask, id response) {
//        self.model = response;
//        NSError *error;
//        completed(error);
...
/** 通过行数, 返回播放时长 秒 */
- (NSTimeInterval )playDurationForRow:(NSInteger)row{
    NSTimeInterval duration = self.model.tracks.list[row].duration;
    return duration;
}
